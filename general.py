...
def box_overlap(box1, box2):
    ...
    # inter(N,M) = (min(right-bottom(N,M,2))) - max(left-top(N,M,2))).clamp(0).prod(2)
    inter = (torch.min(box1[:, None, 2:], box2[:, 2:]) - torch.max(box1[:, None, :2], box2[:, :2])).clamp(0).prod(2)
    return inter / area2


def boxBigEnough(box, validWidth, validHeight):

    # requirement W >= 35, H >= 20
    box=box.T
    # print(box[2]-box[0])
    if (box[2][0]-box[0][0]) >= validWidth:
        if (box[3][0] - box[1][0]) >= validHeight:
            return True
    else: 
        return False
    # return torch.logical_and((box[2] - box[0]) >= validWidth, (box[3] - box[1]) >= validHeight)
